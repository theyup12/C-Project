...
template <class Strategy> struct Strategy_t {
  Bank myChecking_;
  Strategy allBills_;
  double amount_penalties_;

  Strategy_t(const std::string &name = "", double starting = 0.0,
             double salary = 0.0);
...
************************************************/
// std::stack and std::queue both provide capability to view or modify the next
// element to be removed.  std::stack calls is top() while std::queue calls it
// front().  This pair of template functions provides a general concept called
// peek() that delegates to the appropriate top() or front() for a given
// container.
template <typename T> auto &peek(std::stack<T> &container) {
  return container.top();
}

template <typename T> auto &peek(std::queue<T> &container) {
...
Strategy_t<Strategy>::Strategy_t(const std::string &name, double starting,
                                 double salary)
